...
class FactorialLengthSUm {
    //------------> Solution starts here!!
    @SuppressWarnings("Main Logic")
    void solve() {
        for (int tc = 1; tc > 0; tc--) {
            int n = ii(), a[] = iia(n), primes[] = sieveEratosthenes(1000000 + 10);
            long faclen[] = new long[n];
            for (int i = 0; i < n; i++) {
                long f[][] = factorFactorial(a[i], primes);
                for (int j = 0; j < f.length; j++) {
                    faclen[i] += f[j][1];
//                    out.println(f[j][0] + " " + f[j][1]);
                }
            }
            long ans = 0;
    ...
    public static long[][] factorFactorial(int n, int[] primes) {
