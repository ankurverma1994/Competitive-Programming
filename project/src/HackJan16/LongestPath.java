...
class LongestPath {
    ...
    void solve() {
        int n = ii(), color[] = iia(n), a[] = iia(n - 1);
        boolean cake = true;
        for (int i : color) {
            if (i == 1) {
                cake = false;
        ...
        int from[] = new int[n];
        for (int i = 0; i < n - 1; i++) {
            if (color[a[i]] == 1 && color[i + 1] == 1) {
                from[i] = a[i];
                to[i] = i + 1;
            }
        }

        int g[][] = packU(n, from, to, n);
        out.println(cake ? "0" : MaxConnComp(g, n));
    }
    ...
    long MaxConnComp(int graph[][], int n) {
        visit = new boolean[n];
        long ans = 0;
        for (int i = 0; i < n; i++) {
            if (!visit[i]) {
