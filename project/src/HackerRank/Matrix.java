...
class Matrix {
    ...
    void solve() {
        int n = ii(), k = ii();
        triple g[] = new triple[n - 1];
        for (int i = 0; i < n - 1; i++) {
            int x = ii(), y = ii(), w = ii();
            g[i] = new triple(x, y, w);
        }
        Arrays.sort(g);
        boolean machine[] = new boolean[n];
        for (int i = 0; i < k; i++) machine[ii()] = true;
        long ans = 0;
        UF uf = new UF(n);
        for (int i = 0; i < n - 1; i++) {
            triple z = g[i];
            int x = z.x, y = z.y, w = z.w;
            if (machine[uf.find(x)] && machine[uf.find(y)]) {
                ans += w;
            } else {
                uf.union(x, y);
                if (machine[x] || machine[y]) {
                    machine[uf.find(x)] = machine[uf.find(y)] = true;
                }
            }
        }
        out.println(ans);
