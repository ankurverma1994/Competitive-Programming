...
class ProbC {
    ...
    void solve() {
        int n = ii(), m = ii();
        owner = iia(n);
        ArrayList<Integer> country[] = new ArrayList[m];
        for (int i = 0; i < m; i++) country[i] = new ArrayList<>();
        for (int i = 0; i < owner.length; i++) {
//            System.out.println(owner[i]);
            country[owner[i] - 1].add(i);
        }
        ...
        for (int Q = ii(); Q > 0; Q--) {
            int city = ii() - 1, county = ii() - 1;
            int ans = Integer.MAX_VALUE;
            for (int x : country[county]) {
                int lca = lca2(x, city, spar, dep);
                ans = Math.min(ans, d[x] + d[city] - 2 * d[lca]);
            }
            out.println(ans);
        }
    }
    ...
    void dfs(int curr, int par, int len) {
        d[curr] = len;
        for (int next : g[curr]) {
            if (next != par)
                dfs(next, curr, len + 1);
