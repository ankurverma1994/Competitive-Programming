...
import java.math.*;
 ...
 class ProbB {
    ...
    void solve() {
        ...
        repair a[] = new repair[count];
        int j = 0;
        for (int i = 0; i < m; i++) {
            if (!ok[i]) {
                a[j] = new repair(from[i], to[i], cost[i]);
                j++;
            }
        }
        Arrays.sort(a);
        ...
        for (int i = 0; i < count; i++) {
            if (uf.connected(a[i].from, a[i].to)) continue;
            else {
                uf.union(a[i].from, a[i].to);
                ans += a[i].cost;
            }
        }
        out.println(ans);
    ...
    class repair implements Comparable<repair> {
        ...
        repair(int from, int to, int cost) {
            this.from = from;
            this.to = to;
            this.cost = cost;
        }
